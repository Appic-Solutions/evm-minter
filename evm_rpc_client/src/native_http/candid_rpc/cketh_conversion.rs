    ...
    crate::native_http::json::requests::GetLogsParam {
        ...
        topics: value
            ...
            .map(|topic| {
                topic
                    .into_iter()
                    .map(|t| crate::native_http::json::FixedSizeData::new(t.into()))
                    .collect()
            })
            .collect(),
    }
...
pub(super) fn from_log_entries(
    value: Vec<crate::native_http::json::responses::LogEntry>,
) -> Vec<evm_rpc_types::LogEntry> {
    value.into_iter().map(from_log_entry).collect()
}

fn from_log_entry(value: crate::native_http::json::responses::LogEntry) -> evm_rpc_types::LogEntry {
...
pub(super) fn from_fee_history(
    value: crate::native_http::json::responses::FeeHistory,
...
pub(super) fn from_transaction_receipt(
    value: crate::native_http::json::responses::TransactionReceipt,
    ...
    evm_rpc_types::TransactionReceipt {
        ...
        status: value.status.map(|v| match v {
            crate::native_http::json::responses::TransactionStatus::Success => Nat256::from(1_u8),
            crate::native_http::json::responses::TransactionStatus::Failure => Nat256::from(0_u8),
        }),
...
pub(super) fn from_block(
    value: crate::native_http::json::responses::Block,
) -> evm_rpc_types::Block {
...
pub(super) fn from_send_raw_transaction_result(
    ...
    value: crate::native_http::json::responses::SendRawTransactionResult,
) -> evm_rpc_types::SendRawTransactionStatus {
    match value {
        crate::native_http::json::responses::SendRawTransactionResult::Ok => {
            evm_rpc_types::SendRawTransactionStatus::Ok(transaction_hash)
        }
        crate::native_http::json::responses::SendRawTransactionResult::InsufficientFunds => {
            evm_rpc_types::SendRawTransactionStatus::InsufficientFunds
        }
        crate::native_http::json::responses::SendRawTransactionResult::NonceTooLow => {
            evm_rpc_types::SendRawTransactionStatus::NonceTooLow
        }
        crate::native_http::json::responses::SendRawTransactionResult::NonceTooHigh => {
...
fn into_transaction_request(
    ...
    TransactionRequest {
        ...
        nonce: nonce.map(TransactionNonce::from),
        to: to.map(|address| Address::new(address.into())),
        from: from.map(|address| Address::new(address.into())),
        gas: gas.map(GasAmount::from),
        value: value.map(Wei::from),
        input: input.map(into_data),
        gas_price: gas_price.map(WeiPerGas::from),
        max_priority_fee_per_gas: max_priority_fee_per_gas.map(WeiPerGas::from),
        max_fee_per_gas: max_fee_per_gas.map(WeiPerGas::from),
        max_fee_per_blob_gas: max_fee_per_blob_gas.map(WeiPerGas::from),
        ...
        chain_id: chain_id.map(ChainId::from),
    }
}
